...
where $a=-1.3\times 10^{-6}$ $\mathrm{W}\mathrm{m}^{-2}\mathrm{ppbv}^{-1}$, $b=-8.2\times 10^{-6}$ $\mathrm{W}\mathrm{m}^{-2}\mathrm{ppbv}^{-1}$, $c=0.043$, and $N=320$ $ppbv$ is the present-day concentration of N2O which is assumed to remain constant in this study~\citep{ref:etminan2016}. Differentiating Equation~\ref{eqn:df} gives the sensitivity of the forcing to a change in methane concentration,
